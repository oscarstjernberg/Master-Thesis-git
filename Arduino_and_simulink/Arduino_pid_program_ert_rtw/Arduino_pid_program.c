...
void Arduino_pid_program_step(void)
{
  ...
  if (sts > 0) {
     ...
     *  Gain: '<S1>/Gain5'
     */
    Arduino_pid_program_B.DataTypeConversion = (uint8_T)(((uint16_T)
      Arduino_pid_program_P.Gain5_Gain * data) >> 7);
  }
  ...
  data = Arduino_pid_program_B.DataTypeConversion;
...
void Arduino_pid_program_initialize(void)
{
  ...
  Arduino_pid_program_DW.obj_p.DataTypeWidth = 1U;
  ...
  Arduino_pid_program_DW.obj.dataSizeInBytes = 1.0;
  Arduino_pid_program_DW.obj.dataType = 0.0;
  ...
  Arduino_pid_program_B.DataTypeConversion = Arduino_pid_program_P.Out1_Y0;
